...
from ui.dialogs.add_material_type_dialog import AddMaterialTypeDialog
from ui.dialogs.view_details.view_material_type_dialog import ViewMaterialTypeDialog
...
class MaterialTypesTab(BaseTab):
    """
    Material Types tab with admin-only columns and audit-ready CRUD calls.
    ...
    required_permissions = {
        ...
        "add":     "add_material_type",
        "edit":    "edit_material_type",
    ...
    def __init__(self, parent=None, current_user=None):
        ...
        super().__init__(title=_("material_types"), parent=parent, user=u)
        self._ = _
        self.material_types_crud = MaterialTypesCRUD()
    ...
    def reload_data(self):
        ...
        if admin:
            for mt in items:
                cb_id = getattr(mt, "created_by_id", None)
                ub_id = getattr(mt, "updated_by_id", None)
                cb_fallback = getattr(mt, "created_by", None)
                ub_fallback = getattr(mt, "updated_by", None)
        ...
        for mt in items:
            row = {
                "id":       getattr(mt, "id", None),
                "name_ar":  getattr(mt, "name_ar", "") or "",
                "name_en":  getattr(mt, "name_en", "") or "",
                "name_tr":  getattr(mt, "name_tr", "") or "",
                "actions":  mt,
            }
            if admin:
                created_by_rel = getattr(mt, "created_by", None)
                updated_by_rel = getattr(mt, "updated_by", None)
                created_by_id  = getattr(mt, "created_by_id", None)
                updated_by_id  = getattr(mt, "updated_by_id", None)
                ...
                row.update({
                    ...
                    "created_at": str(getattr(mt, "created_at", "") or ""),
                    "updated_at": str(getattr(mt, "updated_at", "") or ""),
                })
    ...
    def display_data(self):
        can_edit = has_perm(self.current_user, "edit_material_type")
        can_delete = has_perm(self.current_user, "delete_material_type")
            ...
            for col_idx, col in enumerate(self.columns):
                ...
                if key == "actions":
                    ...
                    if can_edit:
                        ...
                        btn_edit.clicked.connect(lambda _=False, obj=mt: self._open_edit_dialog(obj))
                        action_layout.addWidget(btn_edit)

                    if can_delete:
                        btn_delete = QPushButton(self._("delete"))
                        btn_delete.setObjectName("danger-btn")
                        btn_delete.clicked.connect(lambda _=False, obj=mt: self._delete_single(obj))
    ...
    def add_new_item(self):
        ...
        if dlg.exec():
            ...
            self.material_types_crud.add_material_type(
    ...
    def edit_selected_item(self, row=None):
        ...
        self._open_edit_dialog(mt)

    def _open_edit_dialog(self, mt):
        dlg = AddMaterialTypeDialog(self, {
            "name_ar": getattr(mt, "name_ar", ""),
            "name_en": getattr(mt, "name_en", ""),
            "name_tr": getattr(mt, "name_tr", ""),
        })
        if dlg.exec():
            data = dlg.get_data()
            user_id = self._user_id()
    ...
    def delete_selected_items(self, rows=None):
        ...
        reply = QMessageBox.question(
            self, self._("delete_material_type"), self._("are_you_sure_delete"),
    ...
    def on_row_double_clicked(self, row_index):
        ...
        if 0 <= row < len(self.data):
            mt = self.data[row]["actions"]
            dlg = ViewMaterialTypeDialog(mt, current_user=self.current_user, parent=self)
            dlg.exec()
    ...
    def retranslate_ui(self):
        ...
        try:
            if parent and hasattr(parent, "indexOf") and hasattr(parent, "setTabText"):
                ...
                if idx != -1:
                    parent.setTabText(idx, self._("material_types"))
        except Exception:
            pass
