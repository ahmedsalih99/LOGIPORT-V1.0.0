...
class PricingTab(BaseTab):
    ...
    def delete_selected_items(self, rows=None):
        ...
        if reply == QMessageBox.Yes:
            ...
            for row in rows:
                p = self.data[row]["actions"]
                ok, err = self._delete_single(p, confirm=False)
                if not ok:
                    _name = getattr(p, "name_ar", None) or getattr(p, "name_en", None) or str(getattr(p, "id", "?"))
    ...
    def _delete_single(self, pricing, confirm=True):
        ...
        try:
            ...
            if "FOREIGN KEY" in err or "IntegrityError" in err:
                msg = self._("pricing_in_use_cannot_delete")
            else:
                msg = err
            if confirm:
